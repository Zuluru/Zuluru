...
if (!empty($offline)) {
	if (Configure::read('registration.online_payments')) {
		echo $this->Html->para(null, __('If you prefer to pay offline (via cheque, email transfer, etc.), the online portion of your registration process is now complete, but you must do the following to make payment:', true));
	} else {
