...
class Zuluru24Schema extends CakeSchema {
		...
		switch ($event['update']) {
			case 'teams':
				...
				foreach (array_keys($teams) as $team_id) {
					// Check all games this team was in and wind back the ratings to find where they started.
	...
	// Other schema changes introduce other tables, but they also introduce
	// dependencies from the Games model, which will cause the migration to
	// fail if the Games model code has been updated to at least that version.
	// There's no harm in adding these tables at this stage in the migration,
	// and it allows the Games model to be created. We do create them with
	// minimal columns, so that any other migration that might have an "after"
	// callback for that table will still happen.
	...
	// The teams table must be listed last, so that the others are
	// all created before we try to run the "after" callback.
	var $teams = array(
		'id' => array('type' => 'integer', 'null' => false, 'default' => NULL, 'key' => 'primary'),
